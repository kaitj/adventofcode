...
def parse_relationship(rules, bag_map):
    for rule in rules.split("\n"):
        ...
        content_re = re.match(
            r"([a-zA-Z ]+) bags contain ((?:\d+ [a-zA-Z ]+ bags?(?:, )?)+)\.", rule)
        ...
        bag_contents = [
            re.match(
                r"(\d+) ([a-zA-Z ]+) bags?",
                content).groups() for content in bag_contents]

        # Store relationships
        for content in bag_contents:
            bag_map[main_bag]["contain"].append(content)
            bag_map[content[1]]["contained by"].append(
                (int(content[0]), main_bag))
...
def main():
    ...
    # Answers
    print(
        f"Number of potential bags able to hold shiny gold bags: {len(gold_parents)}")
