...
def compute_chains(adapter_list):
    ...
    for adapter in adapter_list[1:]:
        chains[adapter] = chains.get(adapter - 1, 0) + \
            chains.get(adapter - 2, 0) + chains.get(adapter - 3, 0)
...
def main():
    ...
    with open(os.path.realpath(adapter_file), "r") as in_file:
        adapter_list = in_file.read().split()
    ...
    print(
        f"The multiplication of 1-jolt & 3-jolt differences is: {adapter_mult}")
