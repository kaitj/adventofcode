...
def apply_mask(mask, value):
    ...
    mask_idx = [idx for idx, mask in enumerate(mask) if mask != "X"]

    for midx in mask_idx:
        if mask[midx] == "1":
            value = value[:midx] + \
                str(int(value[midx]) | int(mask[midx])) + value[midx + 1:]
        else:
            value = value[:midx] + "0" + value[midx + 1:]
...
def update_addr(mask, addr):
    ...
    new_addr = "".join(change_bit[mask[m]](addr_bit[m])
    ...
    for bit_replacement in product(range(2), repeat=len(addr_idx)):
        ...
        for bidx, bit in enumerate(bit_replacement):
            addr_combo[addr_idx[bidx]] = str(bit)
...
def main():
    ...
    with open(os.path.realpath(init_file), "r") as in_file:
        init_prog = [l.strip().split(" = ") for l in in_file]
