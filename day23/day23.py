...
def parse_input(in_fpath: Path) -> dict[str, list[str | None]]:
    with in_fpath.open(encoding="ascii") as in_file:
        c = [
            re.match(r"\W*#+(\w)#(\w)#(\w)#(\w)#+", l).groups()  # type: ignore
            for l in in_file.readlines()[2:4]
        ]
...
if __name__ == "__main__":
    ...
    res = find_path(Burrow(puzzle))
    print(f"Answer: {res.cost}")

    for c in PART_2_INSERTIONS:
        puzzle[c] = [puzzle[c][0]] + PART_2_INSERTIONS[c] + [puzzle[c][1]]
    res = find_path(Burrow(puzzle))
    print(f"Answer: {res.cost}")
